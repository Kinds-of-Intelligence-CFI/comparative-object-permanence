...
children_test_data <- children_wide_data %>%
  select(c(!starts_with("tutorial") & !agent_tag))
...
child_demographic_questions <- read.csv("~/comparative-object-permanence/children/results_children.csv") %>%
  select(c(participant_id, colour_blindness:game_controller_do_not_know))
...
child_game_type_questions <- child_demographic_questions %>%
  select(first_third_person_games:game_type_do_not_know & !game_type_other) 
...
child_game_controller_questions <- child_demographic_questions %>%
  select(desktop_with_keyboard:game_controller_do_not_know) 
...
Next, we do some diagnostic checks to check the differences in performance between the agents. Namely, we look to see if there was a significant difference between males and females, between different age groups, and between random action and random walker agents.
