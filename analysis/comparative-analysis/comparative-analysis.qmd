...
barchart_op_control_chick_choice <- final_results %>%
  mutate(correctChoice = ifelse((cvchickleftchoice == TRUE & cvchickcorrectchoice == "L" & cvchickrightchoice == FALSE)|(cvchickrightchoice == TRUE & cvchickcorrectchoice == "R" & cvchickleftchoice == FALSE), 1, 0)) %>%
  filter(cvchickTask == 1) %>%
...
barchart_op_control_3cup_choice <- final_results %>%
  mutate(correctChoice = ifelse((threecupleftchoice == TRUE & pctb3cupcorrectchoice == "L")|(pctb3cupcorrectchoice == "M" & threecupmidchoice == TRUE)|(pctb3cupcorrectchoice == "R" & threecuprightchoice == TRUE)|(threecupleftchoice == TRUE & threecupmidchoice == TRUE & pctb3cupcorrectchoice == "LM") | (threecuprightchoice == TRUE & threecupmidchoice == TRUE & pctb3cupcorrectchoice == "MR")|(threecuprightchoice == TRUE & threecupleftchoice == TRUE & pctb3cupcorrectchoice == "LR"), 1, 0)) %>%
  filter(pctb3CupTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, correctChoice)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
...
barchart_op_control_grid_choice <- final_results %>%
  mutate(correctChoice = ifelse(pctbgridcorrectchoice == pctbgridcupchoice, 1, 0)) %>%
  filter(pctbGridTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, correctChoice)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
