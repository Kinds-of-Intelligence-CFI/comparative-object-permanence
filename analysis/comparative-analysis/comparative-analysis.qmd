...
pal <- c("#d95f02", "#7570b3", "#4b7b16", "#59901a", "#66a61e", "#73bc22", "#81d126", "#c71670", "#de197d", "#e7298a", "#ea4097", "#ec57a3", "#e6ab02") # color palette for plots
...
density_bar_plotter <- function(task_data){
...
This document presents the descriptive statistics and and comparative analytics for the agents studied in the `comparative-object-permanence` study. This document does not contain any psychometric analysis (measurement layouts). Below, '*agents*' refers to artificial agents like random agents or deep reinforcement learners. '*children*' refers to human children (!). Note that all floats are reported to `r rounding` decimal places.

```{r data import}

final_results <- read.csv("../results_final_clean_long.csv") %>%
  filter(problem_flag == "N") %>% #remove the problematic data points.
  mutate(agent_type = ifelse(agent_type == "dreamer-bc-all", "Dreamer 1",
                             ifelse(agent_type == "dreamer-bc_opc-strat", "Dreamer 2",
                                    ifelse(agent_type == "dreamer-bc_opc-all", "Dreamer 3",
                                           ifelse(agent_type == "dreamer-bc_opc_opt-strat", "Dreamer 4",
                                                  ifelse(agent_type == "dreamer-bc_opc_opt-all", "Dreamer 5",
                                                         ifelse(agent_type == "ppo-bc-all", "PPO 1",
                             ifelse(agent_type == "ppo-bc_opc-strat", "PPO 2",
                                    ifelse(agent_type == "ppo-bc_opc-all", "PPO 3",
                                           ifelse(agent_type == "ppo-bc_opc_opt-strat", "PPO 4",
                                                  ifelse(agent_type == "ppo-bc_opc_opt-all", "PPO 5", agent_type)))))))))))
...
agent_data <- final_results %>% 
  dplyr::select(c(agent_tag, agent_type, agent_type_gen, aai_seed, age, gender)) %>% distinct()

random_agent_distinct <- agent_data %>% dplyr::select(agent_tag, agent_type) %>%
  filter(agent_type == "Random Agent") %>% distinct()

heuristic_distinct <- agent_data %>% dplyr::select(agent_tag, agent_type) %>%
  filter(agent_type == "Heuristic Agent") %>% distinct()

ppo_distinct <- agent_data %>% dplyr::select(agent_tag, agent_type_gen) %>%
  filter(agent_type_gen == "PPO") %>% distinct()

dreamer_distinct <- agent_data %>% dplyr::select(agent_tag, agent_type_gen) %>%
  filter(agent_type_gen == "Dreamer") %>% distinct()

children_distinct <- agent_data %>% dplyr::select(agent_tag, agent_type) %>%
  filter(agent_type == "Child") %>% distinct()
...
-   `r nrow(random_agent_distinct)` types of random agent, tested on 5 RNG seeds each. Total sample size: 25.
-   `r nrow(heuristic_distinct)` type of simple goal-directed rule-based agent, tested on 5 RNG seeds. Total sample size: 5.
-   `r nrow(ppo_distinct)` types of PPO agent, tested on 1 RNG seed.
-   `r nrow(dreamer_distinct)` types of dreamer agent, tested on 1 RNG seed.
...
For the `r nrow(performances_table)` kinds of agents, the number of instances that they passed, as well as the proportion of total instances that they passed, is provided in the table below. The behaviour of the random agents serve as a baseline for chance performance.

```{r overall success table}

datatable(performances_table, rownames = FALSE, options = list(pageLength = nrow(performances_table)))
...
datatable(proportions, rownames = FALSE, options = list(pageLength = nrow(proportions)))
...
raincloud_data <- final_results %>%
  dplyr::select(c(agent_tag, agent_type, proportionSuccess)) %>%
  mutate(agent_type = haven::as_factor(agent_type)) %>%
  group_by(agent_type)

density_bar_plotter(raincloud_data)
...
basic_tasks %>%
  ...
  summarise(`Total Passes` = sum(success),
            ...
            `Percentage Instances Passed` = round(`Total Passes`/`Total Instances`*100, digits = rounding)) %>%
  arrange(desc(`Percentage Instances Passed`)) %>%
  rename(Agent = agent_type) %>% datatable(rownames = FALSE, options = list(pageLength = nrow(.)))
...
density_bar_plotter(basic_tasks)
...
density_bar_plotter(control_tasks)
...
```{r CV Chick Control Success Overview}

cv_control_tasks <- final_results %>% 
...
```{r cv control tasks success rates}
...
```{r proportion scores cv control tasks}
...
```{r proportion scores graph cv control tasks}

density_bar_plotter(cv_control_tasks)

```

We can also examine the way that the agents failed these tasks. In the table, we provide the percentage of tasks that ended in certain ways for each agent:

```{r cv control tasks episode end types}

cv_control_tasks %>% group_by(agent_type, lavaPresence, Paradigm) %>%
  summarise(`Total Instances` = n(),
            `Lava` = round(sum(episodeEndType == "lava")/`Total Instances`*100, 4),
            `Time` = round(sum(episodeEndType == "time")/`Total Instances`*100, 4),
            `Passed` = round(sum(episodeEndType == "all goal(s) obtained")/`Total Instances`*100, 4)) %>%
...
cvchick_choices <- final_results %>%
  filter((Suite == "Controls" | Suite == "Tutorial") & SubSuite == "OP Controls" & Paradigm == "CVChick") %>%
  dplyr::select(c(agent_type, cvchickcorrectchoice, cvchickleftchoice, cvchickrightchoice, success, correctChoice)) %>% drop_na()
...
cvchick_choices_table <- cvchick_choices %>% group_by(agent_type) %>%
  summarise(`Total Instances` = n(),
            ...
            `Percentage Correct Choice` = round(sum(correctChoice)/`Total Instances`*100, digits = rounding),
...
datatable(cvchick_choices_table, rownames = FALSE, options = list(pageLength = nrow(cvchick_choices_table)))
...
```{r Cup Chick Control Success Overview}

cup_control_tasks <- final_results %>% 
...
First, the success rates across this subset of tests is provided.

```{r cup control tasks success rates}
...
```{r proportion scores cup control tasks}
...
```{r proportion scores graph cup control tasks}

density_bar_plotter(cup_control_tasks)

```

We can also examine the way that the agents failed these tasks. In the table, we provide the percentage of tasks that ended in certain ways for each agent:

```{r cup control tasks episode end types}

cup_control_tasks %>% group_by(agent_type, lavaPresence, Paradigm) %>%
  summarise(`Total Instances` = n(),
            `Lava` = round(sum(episodeEndType == "lava")/`Total Instances`*100, 4),
            `Time` = round(sum(episodeEndType == "time")/`Total Instances`*100, 4),
            `Passed` = round(sum(episodeEndType == "all goal(s) obtained")/`Total Instances`*100, 4)) %>%
...
pctb3cup_choices <- final_results %>%
  filter((Suite == "Controls" | Suite == "Tutorial") & SubSuite == "OP Controls" & str_detect(Task, "3Cup")) %>%
  dplyr::select(c(agent_type, threecupleftchoice, threecupmidchoice, threecuprightchoice, pctb3cupcorrectchoice, success, correctChoice)) %>% drop_na()
...
datatable(pctb3cup_choices_table, rownames = FALSE, options = list(pageLength = nrow(pctb3cup_choices_table)))
...
```{r Grid Chick Control Success Overview}

grid_control_tasks <- final_results %>% 
...
```{r grid control tasks success rates}
...
```{r proportion scores grid control tasks}
...
```{r proportion scores graph grid control tasks}

density_bar_plotter(grid_control_tasks)

```

We can also examine the way that the agents failed these tasks. In the table, we provide the percentage of tasks that ended in certain ways for each agent:

```{r grid control tasks episode end types}

grid_control_tasks %>% group_by(agent_type, lavaPresence, Paradigm) %>%
  summarise(`Total Instances` = n(),
            `Lava` = round(sum(episodeEndType == "lava")/`Total Instances`*100, 4),
            `Time` = round(sum(episodeEndType == "time")/`Total Instances`*100, 4),
            `Passed` = round(sum(episodeEndType == "all goal(s) obtained")/`Total Instances`*100, 4)) %>%
...
pctbgrid_choices <- final_results %>%
  filter((Suite == "Controls" | Suite == "Tutorial") & SubSuite == "OP Controls" & str_detect(Task, "Grid")) %>%
  dplyr::select(c(agent_type, InstanceName, pctbgridcorrectchoice, pctbgridcupchoice, success, correctChoice, numChoices)) %>% drop_na()

pctbgrid_choices_table <- pctbgrid_choices %>% group_by(agent_type) %>%
  summarise(`Total Instances` = n(),
            `Percentage Instances Passed` = round(sum(success)/`Total Instances`*100, digits = rounding),
            `Percentage Correct Choice` = round(sum(correctChoice)/`Total Instances`*100, digits = rounding),
...
datatable(pctbgrid_choices_table, rownames = FALSE, options = list(pageLength = nrow(pctbgrid_choices_table)))

```


Broken down by number of cups:

```{r PCTB Grid Control tasks overview by cups}
...
cv_tasks <- final_results %>% 
  ...
  dplyr::select(c(agent_tag, agent_type, proportionSuccess, success, correctChoice, episodeEndType, lavaPresence, Paradigm, cvchickleftchoice, cvchickrightchoice, cvchickcorrectchoice)) %>%
  mutate(agent_type = haven::as_factor(agent_type))
...
threecup_tasks <- final_results %>% 
  ...
  dplyr::select(c(agent_tag, agent_type, proportionSuccess, success, correctChoice, episodeEndType, lavaPresence, Paradigm, threecupleftchoice, threecupmidchoice, threecuprightchoice, pctb3cupcorrectchoice)) %>%
  mutate(agent_type = haven::as_factor(agent_type))
...
density_bar_plotter(threecup_tasks)
...
grid_tasks <- final_results %>% 
  ...
  dplyr::select(c(agent_tag, agent_type, proportionSuccess, success, correctChoice, episodeEndType, Paradigm, pctbgridcorrectchoice, pctbgridcupchoice, numChoices)) %>%
  mutate(agent_type = haven::as_factor(agent_type))
...
density_bar_plotter(grid_tasks)
...
pctbgrid_choices_table <- grid_tasks %>% group_by(agent_type) %>%
  summarise(`Total Instances` = n(),
            `Percentage Instances Passed` = round(sum(success)/`Total Instances`*100, digits = rounding),
            `Percentage Correct Choice` = round(sum(correctChoice)/`Total Instances`*100, digits = rounding),
            `Percentage Incorrect Choice` = round(sum(pctbgridcorrectchoice != pctbgridcupchoice & pctbgridcupchoice != "NoChoiceMade")/`Total Instances`*100, digits = rounding),
...
```{r PCTB Grid OP tasks overview by cups}
...
barchart_success_age_data <- final_results %>%
  ...
  mutate(agent_type = ifelse(agent_type == "Child" & age == 4, "4 Year Old",
...
```{r basic-success-barchart}

barchart_basic_success <- final_results %>%
  filter(basicTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, basicTask, success)) %>%
  mutate(`Task Type` = ifelse(basicTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
         agent_type = factor(agent_type, levels = c("Random Agent", "Heuristic Agent", "Children", "PPO 1", "PPO 2", "PPO 3", "PPO 4", "PPO 5", "Dreamer 1", "Dreamer 2", "Dreamer 3", "Dreamer 4", "Dreamer 5"), ordered = TRUE)) %>%
  group_by(agent_type, `Task Type`) %>%
  summarise(`Percentage Success` = (sum(success)/n())*100)  %>%
  rename(`Agent` = agent_type)

(barchart_basic_success <- barchart_basic_success %>%
  ggplot(aes(x = Agent, y = `Percentage Success`, fill = `Task Type`)) + geom_col(position = "dodge", width = 0.5) +
    ylim(0,100)  + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
### Comparing OP Tests and Controls

Here are some bar charts directly comparing the OP and Control tasks for each experimental paradigm.

Chick tasks:

```{r op-control-chick-task-success}

barchart_op_control_chick_success <- final_results %>%
  filter(cvchickTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, success)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
         agent_type = factor(agent_type, levels = c("Random Agent", "Heuristic Agent", "Children", "PPO 1", "PPO 2", "PPO 3", "PPO 4", "PPO 5", "Dreamer 1", "Dreamer 2", "Dreamer 3", "Dreamer 4", "Dreamer 5"), ordered = TRUE)) %>%
  group_by(agent_type, `Task Type`) %>%
  summarise(`Percentage Success` = (sum(success)/n())*100)  %>%
...
(barchar_op_control_chick_success <- barchart_op_control_chick_success %>%
  ggplot(aes(x = Agent, y = `Percentage Success`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)
...
```{r op-control-chick-task-choice}

barchart_op_control_chick_choice <- final_results %>%
  filter(cvchickTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, correctChoice)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
         agent_type = factor(agent_type, levels = c("Random Agent", "Heuristic Agent", "Children", "PPO 1", "PPO 2", "PPO 3", "PPO 4", "PPO 5", "Dreamer 1", "Dreamer 2", "Dreamer 3", "Dreamer 4", "Dreamer 5"), ordered = TRUE)) %>%
  group_by(agent_type, `Task Type`) %>%
  summarise(`Percentage Correct Choice` = (sum(correctChoice)/n())*100)  %>%
  rename(`Agent` = agent_type)

(barchart_op_control_chick_choice <- barchart_op_control_chick_choice %>%
  ggplot(aes(x = Agent, y = `Percentage Correct Choice`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
```{r op-control-cup-task-success}

barchart_op_control_3cup_success <- final_results %>%
  filter(pctb3CupTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, success)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
         agent_type = factor(agent_type, levels = c("Random Agent", "Heuristic Agent", "Children", "PPO 1", "PPO 2", "PPO 3", "PPO 4", "PPO 5", "Dreamer 1", "Dreamer 2", "Dreamer 3", "Dreamer 4", "Dreamer 5"), ordered = TRUE)) %>%
...
(barchart_op_control_3cup_success <- barchart_op_control_3cup_success %>%
  ggplot(aes(x = Agent, y = `Percentage Success`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
```{r op-control-cup-task-choice}

barchart_op_control_3cup_choice <- final_results %>%
  filter(pctb3CupTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, correctChoice)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
...
(barchart_op_control_3cup_choice <- barchart_op_control_3cup_choice %>%
  ggplot(aes(x = Agent, y = `Percentage Correct Choice`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
```{r op-control-grid-task-success}

barchart_op_control_grid_success <- final_results %>%
  filter(pctbGridTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, success)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
         agent_type = factor(agent_type, levels = c("Random Agent", "Heuristic Agent", "Children", "PPO 1", "PPO 2", "PPO 3", "PPO 4", "PPO 5", "Dreamer 1", "Dreamer 2", "Dreamer 3", "Dreamer 4", "Dreamer 5"), ordered = TRUE)) %>%
...
(barchart_op_control_grid_success <- barchart_op_control_grid_success %>%
  ggplot(aes(x = Agent, y = `Percentage Success`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
```{r op-control-grid-task-choice}

barchart_op_control_grid_choice <- final_results %>%
  filter(pctbGridTask == 1) %>%
  dplyr::select(c(agent_tag, agent_type, opControlTask, correctChoice)) %>%
  mutate(`Task Type` = ifelse(opControlTask == 1, "Control", "OP Test"),
         agent_type = ifelse(agent_type == "Child", "Children", agent_type),
...
(barchart_op_control_grid_choice <- barchart_op_control_grid_choice %>%
  ggplot(aes(x = Agent, y = `Percentage Correct Choice`, fill = `Task Type`)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
...
## DRL-Specific Descriptives

Finally, let's look at the differences between the curricula

```{r curriculum comparisons}

barchart_curriculum_comparisons <- final_results %>%
  filter(str_detect(agent_tag, "ppo|dreamer")) %>%
  transmute(success = success,
            `Algorithm` = ifelse(str_detect(agent_tag, "ppo"), "PPO", "Dreamer"),
            `Curriculum` = ifelse(str_detect(agent_tag, "bc-all"), "(1) All Basic",
                                  ifelse(str_detect(agent_tag, "bc_opc-strat"), "(2) Basic + Strat.\nControl",
                                         ifelse(str_detect(agent_tag, "bc_opc-all"), "(3) Basic + All\nControl",
                                                ifelse(str_detect(agent_tag, "bc_opc_opt-strat"), "(4) Basic + Strat.\nControl + Strat\nOP",
                                                       ifelse(str_detect(agent_tag, "bc_opc_opt-all"), "(5) Basic + All\nControl + Strat\nOP", agent_tag))))),
            Algorithm = factor(Algorithm, levels = c("PPO", "Dreamer", ordered = TRUE)),
            Curriculum = factor(Curriculum, levels = c("(1) All Basic", "(2) Basic + Strat.\nControl", "(3) Basic + All\nControl", "(4) Basic + Strat.\nControl + Strat\nOP", "(5) Basic + All\nControl + Strat\nOP"), ordered = TRUE)) %>%
  group_by(Algorithm, Curriculum) %>%
  summarise(`Percentage Success` = (sum(success)/n())*100)  

(barchart_curriculum_comparisons_plot <- barchart_curriculum_comparisons %>%
  ggplot(aes(x = Curriculum, y = `Percentage Success`, fill = Algorithm)) + geom_col(position = "dodge") +
    ylim(0,100) +
    #scale_fill_manual(values = barchart_palette) + 
  theme_minimal(base_family = "Arial", base_size = 15) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)
