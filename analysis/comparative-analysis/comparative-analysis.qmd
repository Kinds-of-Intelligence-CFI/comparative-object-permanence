...
Next, we present some simple frequentist statistics, to explore the data before doing the more complex measurement layouts. We ran Kruskall-Wallis tests on each type of task, to see investigate the magnitude of the difference between the groups.

```{r kruskall-wallis tests success}

basic_tasks_KW_test <- final_results %>%
  filter((Suite == "Controls" | Suite == "Tutorial") & SubSuite == "Basic") %>%
  select(agent_tag, agent_type, age, aai_seed, success) %>%
  mutate(agent_tag_seed = haven::as_factor(paste0(agent_tag, "_", aai_seed)),
         agent_type = ifelse(is.na(age), agent_type, age)) %>%
...
```{r kruskall-wallis tests correctChoice}

basic_tasks_KW_test <- final_results %>%
  filter((Suite == "Controls" | Suite == "Tutorial") & SubSuite == "Basic") %>%
  select(agent_tag, agent_type, age, aai_seed, correctChoice) %>%
  mutate(agent_tag_seed = haven::as_factor(paste0(agent_tag, "_", aai_seed)),
         agent_type = ifelse(is.na(age), agent_type, age)) %>%
  group_by(agent_tag_seed, agent_type) %>%
